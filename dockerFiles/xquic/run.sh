...
# EDIT THESE (those in "")
TARGET_DIR=${TARGET_DIR:-"xquic"}
COVERAGE_DIR=${COVERAGE_DIR:-"xquic_cov/build"}
INPUTS=${INPUTS:-"${WORKDIR}/xquic_seed"}
SERVER_CMD="/tmp/xquic/build/tests/test_server -a 127.0.0.1 -p 4433 -o /dev/null"
# SERVER_CMD="/tmp/xquic/build/tests/test_server -a 127.0.0.1 -p 4433 -l error -o /dev/null"
COV_BIN="/tmp/xquic_cov/build/tests/test_server"
