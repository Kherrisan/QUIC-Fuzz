...
# EDIT THESE (those in "")
TARGET_DIR=${TARGET_DIR:-"lsquic"}
COVERAGE_DIR=${COVERAGE_DIR:-"lsquic_cov"}
INPUTS=${INPUTS:-"${WORKDIR}/lsquic_seed"}
SERVER_CMD="/tmp/lsquic/bin/http_server -Q hq-29 -s 127.0.0.1:4433 -c www.example.com,/tmp/server-cert.pem,/tmp/server-key.pem"
