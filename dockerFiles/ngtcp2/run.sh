...
# EDIT THESE (those in "")
TARGET_DIR=${TARGET_DIR:-"ngtcp2"}
COVERAGE_DIR=${COVERAGE_DIR:-"ngtcp2_cov"}
INPUTS=${INPUTS:-"${WORKDIR}/ngtcp2_seed"}
SERVER_CMD="/tmp/ngtcp2/examples/wsslserver 127.0.0.1 4433 /tmp/server-key.pem /tmp/server-cert.pem --initial-pkt-num=0"
COV_BIN="/tmp/ngtcp2_cov/examples/wsslserver"
