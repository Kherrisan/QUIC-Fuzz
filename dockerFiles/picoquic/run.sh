...
# EDIT THESE (those in "")
TARGET_DIR=${TARGET_DIR:-"picoquic"}
COVERAGE_DIR=${COVERAGE_DIR:-"picoquic_cov"}
INPUTS=${INPUTS:-"${WORKDIR}/picoquic_seed"}
SERVER_CMD="/tmp/picoquic/picoquicdemo -R 0 -p 4433 -c /tmp/server-cert.pem -k /tmp/server-key.pem"
COV_BIN="/tmp/picoquic_cov/picoquicdemo"
