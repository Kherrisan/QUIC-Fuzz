...
int main(int argc, char* argv[])
{
  ...
  else if (!strcmp(argv[2], "QUIC")) extract_response_codes = &extract_response_codes_quic;
  ...
  // extract the secrets for encryption
  if(strcmp(argv[2], "QUIC") == 0){
    is_quic = 1;
    init_quic_info();

    // get the seed file
    u8 *secret_filename = alloc_printf("%s.secret", argv[1]);

    if(access(secret_filename, F_OK) != -1){
      read_secrets_from_file(secret_filename);
  ...
  int sockfd;
  if (is_quic || (!strcmp(argv[2], "DTLS12")) || (!strcmp(argv[2], "DNS")) || (!strcmp(argv[2], "SIP"))) {
  ...
  while(!feof(fp)) {
    ...
    if (fread(&size, sizeof(unsigned int), 1, fp) > 0) {
      ...
      // use for debugging (chose which packet to send to reduce the input sequence triggering a crash)
      //if (packet_count != 1 && packet_count != 4 && packet_count != 5){
      // n = net_send(sockfd, timeout, buf,size);
      // if (n != size) break;
      //}
