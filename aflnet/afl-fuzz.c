...
EXP_ST u8 *in_dir,                    /* Input directory with test cases  */
          ...
          *sbr_bin_path;              /* Path to SaBRe binary             */ // QUIC-Fuzz ADDED HERE
...
EXP_ST u8  skip_deterministic,        /* Skip deterministic stages?       */
           ...
           sbr_bin_given,             /* Is the sabre binary path given?  */ // QUIC-Fuzz ADDED HERE
...
// QUIC-Fuzz: snapfuzz benchmark stuff
// #ifdef PRINT_BENCH
...
static TargetAction target_will_do() {
  ...
  do {
    ...
    if (child_timed_out || stop_soon) {
      // WARNF("Child timeout!!! %d", child_timed_out);
      // WARNF("Stop soon!!! %d", stop_soon);
      return Timeout;
    }
  } while (rc <= 0);
  return ta;
...
int send_over_network_sbr() {
  ...
  for (kliter_t(lms) *it = kl_begin(kl_messages); it != kl_end(kl_messages); it = kl_next(it)) {
    ...
    TOKF("2 Send: %d msg: %s", n, kl_val(it)->mdata);
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  if (dumb_mode == 1 || no_forkserver) {
    ...
    if ((res = write(fsrv_ctl_fd, &prev_timed_out, 4)) != 4) {
      ...
      RPFATAL(res, "Unable to request new process from fork server when write (OOM?)");
    ...
    if ((res = read(fsrv_st_fd, &child_pid, 4)) != 4) {
      ...
      if (stop_soon) return 0;
      RPFATAL(res, "Unable to request new process from fork server when read (OOM?)");
  ...
  if (WIFSIGNALED(status) && !stop_soon) {
    ...
    if (kill_signal == SIGINT && ignore_sigint) return FAULT_NONE;
...
static void perform_dry_run(char** argv) {
  ...
  set_stop_derive_initial_secret();
...
static void maybe_delete_out_dir(void) {
  ...
  /* Delete decrypted seed. */

  fn = alloc_printf("%s/decrypted_seed", out_dir);
...
EXP_ST void check_binary(u8* fname) {
  ...
  if (memmem(f_data, f_len, DEFER_SIG, strlen(DEFER_SIG) + 1) || sbr_mode) {
    ...
    if(sbr_mode){
      OKF(cPIN "Smart deferred forkserver will be enabled if the target is compiled with LLM mode.");
    }else{
...
static void usage(u8* argv0) {
  ...
  SAYF("\n%s [ options ] -- /path/to/fuzzed_app [ ... ]\n\n"
       ...
       "  -P protocol   - application protocol to be tested (e.g., RTSP, FTP, DTLS12, DNS, SMTP, SSH, TLS, QUIC)\n"
       ...
       "  -y                    - enable decryption and encryption during fuzzing\n"
       "  -Y                    - generate decrypted seed and exit\n"
       "  -A snapfuzz_plugin    - use snapfuzz plugin to improve stability and execution speed\n"
       "  -a sabre_binary       - sabre binary that hook syscalls handled by the snapfuzz_plugin\n"
       "  -p                    - value to return when skipping recvmsg(), use this with snapfuzz plugin (default: 0)\n"
...
EXP_ST void setup_dirs_fds(void) {
  ...
  /* QUIC-Fuzz ADDED HERE */
  /* store decrypted seed, will be used as in_dir later */
  if(is_fuzz_with_dec_enc){
    tmp = alloc_printf("%s/decrypted_seed", out_dir);
...
/* QUIC-Fuzz ADDED HERE */
/* process the raw seed when -y flag is set*/
EXP_ST void process_raw_seed(void){
  unsigned int no_quic_pkt = 1;
  struct dirent **nl;
  s32 nl_cnt;
  
  ACTF("Processing '%s'...", in_dir);
  ...
  for (int i = 0; i < nl_cnt; i++) {
    ...
    // decrypt the raw seed and store it
    if(is_gen_train_data){
      no_quic_pkt = convert_raw_to_decrypted_seed(fn, NULL);
    }else if(is_fuzz_with_dec_enc){
      u8* new_fn = alloc_printf("%s/decrypted_seed/%s", out_dir, nl[i]->d_name);
      no_quic_pkt = convert_raw_to_decrypted_seed(fn, new_fn);
      ck_free(new_fn);
    }
  ...
  if(no_quic_pkt && is_fuzz_with_dec_enc){
    // incase all seed does not has quic packet, disable is_fuzz_with_dec_enc, just fuzz with raw seed
    is_fuzz_with_dec_enc = 0;
    // set file
  }else if(is_fuzz_with_dec_enc){
    in_dir = alloc_printf("%s/decrypted_seed", out_dir);
  }
}
...
static char** get_sbr_argv(char** argv, int argc) {
  ...
  char** new_argv = ck_alloc(sizeof(char*) * (argc + extra_argc + 1));

  memcpy(new_argv + extra_argc, argv, sizeof(char*) * argc);

  if(snapfuzz_args){
    new_argv[3] = "--";
    new_argv[2] = snapfuzz_args;
  }else{
    new_argv[2] = "--";
  }
  ...
  if(access(sbr_plugin_path, F_OK) != 0){
    FATAL("The Snapfuzz plugin provided is not found!");
  }

  if(sbr_bin_given){
    if(access(sbr_bin_path, F_OK) != 0){
      FATAL("The Sabre binary provided is not found!");
    }

    target_path = new_argv[0] = ck_strdup(sbr_bin_path);
  }else{
    if(access("./sabre", F_OK) != 0){
      FATAL("The Sabre binary provided is not found!");
    }
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:QN:a:A:D:W:w:e:P:KEq:s:RFc:l:yYb:Ip:")) > 0)
    ...
    switch (opt) {
      ...
      case 'a': /* SaBRe binary path */
        if (sbr_bin_given) FATAL("Multiple -a options not supported");
        sbr_bin_path = optarg;
      ...
      // QUIC-Fuzz ADDED HERE
      case 'y': /* dry run only */
        if (is_fuzz_with_dec_enc) FATAL("Multiple -y options not supported");
        if (is_gen_train_data) FATAL("Not able to run with -Y options.");
        is_fuzz_with_dec_enc = 1;
        break;

      // QUIC-Fuzz ADDED HERE
      case 'Y': /* generate training data */
        if (is_gen_train_data) FATAL("Multiple -Y options not supported");
        if (is_fuzz_with_dec_enc) FATAL("Not able to run with -y options.");
        is_gen_train_data = 1;
        break;
      ...
      // QUIC-Fuzz ADDED HERE
      case 'p': /* Snapfuzz plugin args */
          if (snapfuzz_args) FATAL("Multiple -p options not supported");
            snapfuzz_args = optarg;
          break;
  ...
  if(is_fuzz_with_dec_enc || is_gen_train_data){
    ...
    // end once all the training data is generated
    if(is_gen_train_data){
      goto stop_fuzzing;
    }
  }
  ...
  if (state_aware_mode) {
    ...
    if (state_ids_count == 0) {
      PFATAL("No server states have been detected. Server responses are likely empty!\n"                    
              "                    If you are running with snapfuzz plugin, try running with \"-p -1\" or \"-p 0\"");
    }
