...
// ADDED HERE for QUIC
// byte_count is used to keep track of how many bytes the function has read from the initial seed input (.raw file).
// mem_count keeps track of how many bytes the function has used to store intermediate data for the current region/message. This variable is reset once the function reaches the end of a message.
// region_count keeps track of the number of messages/regions the function so far has extracted from the seed input.
  ...
  while (byte_count < buf_size) {
    ...
    if(mem_count == 5){
      ...
      if(is_long_head_pkt(mem, mem_count)){
        ...
        // in retry mode, if the first packet is Initial, we do not need to decrypt it because it is use to trigger a Retry from the packet.
        if(!(is_retry && region_count == 0 && packet_type == Initial) && is_fuzz_with_dec_enc){
...
unsigned int* extract_response_codes_quic(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref){
  ...
  while (byte_count < buf_size) {
    ...
    if(mem_count == 5){
      unsigned int temp_count = 0;
      unsigned int bytes_to_skip = 0;
      unsigned int packet_num_offset;
      ...
      if(is_long_head_pkt(mem, mem_count)){
        ...
        if((mem[0] & (1 << 7))){
          ...
          if(packet_type == Initial){
            // get the correct Initial key for decryption
            if(quic_info.initial_secret != NULL && is_fuzz_with_dec_enc){
              for(int i=0; i<quic_info.initial_secret_count; i++){
                // check length
                if(quic_info.initial_secret[i].client_source_conn_id_len == dcidLen){
                  // check cid
      ...
      if(is_extracted_packet){
        ...
        unsigned int decrypted_packet_len = 0;
        unsigned int frame_type = 0;

        if(is_fuzz_with_dec_enc){
          decrypted_packet_len = decrypt_packet(0, mem, mem_count+1, packet_num_offset, packet_type, &decrypted_packet, &frame_type);
        }

        if(decrypted_packet) ck_free(decrypted_packet);
        ...
        unsigned int message_code = (packet_type << 30) | frame_type;
        if(!is_fuzz_with_dec_enc){
          message_code += 1;  // prevent the message code become 0,, which is same as initial state.
        }
        state_count++;
        state_sequence = (unsigned int *)ck_realloc(state_sequence, state_count * sizeof(unsigned int));
  ...
  for (it = kl_begin(kl_messages); it != kl_end(kl_messages) && message_count < max_count; it = kl_next(it)) {
    ...
    if (replay_enabled) {
      ...
      *psize = message_size;
      ...
      // QUIC-Fuzz ADDED HERE
      // write the secret of crashing input to a file (to re-encrypt when replay)
      if(is_quic && is_fuzz_with_dec_enc){
        u8* secret_filename = alloc_printf("%s.secret", fname);
        write_secrets_to_file(secret_filename);
        ck_free(secret_filename);
      }
    } else {
...
int net_send(int sockfd, struct timeval timeout, char *mem, unsigned int len) {
  ...
  unsigned char *encrypted_packet = NULL;
  unsigned int is_encrypted = 0;

  // try to encrypt here before sending if is_fuzz_with_dec_enc is set
  // if encryption failed, directly send the pain text
  // else send the encrypted packet
  if(is_quic && is_fuzz_with_dec_enc){
    enum PacketType packet_type = get_packet_type(mem, len);
    unsigned int packet_num_offset;
    unsigned int encrypted_packet_len = 0;

    if(packet_type != Error){
      if(packet_type == OneRTT){
        packet_num_offset = get_one_rtt_pkt_num_offset(1, mem, len);
      }else{
        packet_num_offset =  get_long_head_pkt_num_offset(mem, len, packet_type);
      }

      encrypted_packet_len = encrypt_packet(1, mem, len, packet_num_offset, packet_type, &encrypted_packet);
    }

    if(encrypted_packet_len > 0){
      len = encrypted_packet_len;
      is_encrypted = 1;

      // FILE *crash_input = fopen("xquic_crash", "ab");
      // fwrite(&len, sizeof(unsigned int), 1, crash_input);
      // fwrite(encrypted_packet, len, 1, crash_input);
      // fclose(crash_input);
    }else{
      // FILE *crash_input = fopen("xquic_crash", "ab");
      // fwrite(&len, sizeof(unsigned int), 1, crash_input);
      // fwrite(mem, len, 1, crash_input);
      // fclose(crash_input);
    }
  }
  ...
  if (rv > 0) {
    if (pfd[0].revents & POLLOUT) {
      while (byte_count < len) {
        ...
        if(is_quic && is_encrypted){
          n = send(sockfd, &encrypted_packet[byte_count], len - byte_count, MSG_NOSIGNAL);
        }else{
          n = send(sockfd, &mem[byte_count], len - byte_count, MSG_NOSIGNAL);
        }

        if (n == 0){
          if(encrypted_packet) ck_free(encrypted_packet);
          return byte_count;
        } 
  ...
  if(is_quic && is_encrypted){
    // minus this authentication tag size which is added after encryption.
    return (byte_count - AUTH_TAG_SIZE);
  }else{
    return byte_count;
  }
}
...
int net_recv(int sockfd, struct timeval timeout, int poll_w, char **response_buf, unsigned int *len) {
  // QUIC-Fuzz: increase the buffer because QUIC packets are quite huge
  char temp_buf[5000];
  int n;
  struct pollfd pfd[1];
  pfd[0].fd = sockfd;
...
int net_send_sbr(int sockfd, char *mem, unsigned int len)
{
  ...
  // if encryption failed, directly send the pain text
  // else send the encrypted packet
  if(is_quic && is_fuzz_with_dec_enc && len > 0){
    enum PacketType packet_type = get_packet_type(mem, len);
    unsigned int packet_num_offset;
    unsigned int encrypted_packet_len = 0;
  ...
  if(is_quic && is_encrypted){
    byte_count = send(sockfd, encrypted_packet, len, MSG_NOSIGNAL);
  }else{
    byte_count = send(sockfd, mem, len, MSG_NOSIGNAL);
  }

  if(encrypted_packet) ck_free(encrypted_packet);

  if(is_quic && is_encrypted && byte_count > 0){
    // minus this authentication tag size which is added after encryption.
    return (byte_count - AUTH_TAG_SIZE);
  }
...
// we use 5KB, increase this will likely to decrease execution speed
static char temp_buf[5120] = {0};
